...
Over the 273-day holdout window, the model predicts **16,867** purchases against **19,684** actual: **-14.3%**.

**On the aggregate total, the model does not beat the best naive rule.** 1 of the 4 naive rules that forecast purchases lands closer. Predicting that each customer keeps repeating at the rate of their last 91 days of calibration gives 21,585 (+9.7%). The 91-day window was chosen after an audit had reported 30, 61 and 91 days, so it was not a blind choice; it is the least favourable of those to the rule. Across look-backs of 30 days +2.6%, 61 days +1.2%, 91 days +9.7%, 100 days +15.1%, 122 days +16.5%, 152 days +18.5%, 182 days +26.6%, 273 days +47.4%, **the rule beats the model only with a short look-back.** It wins at 91 days and loses from 100 days on, so the crossover lies between the two. The result is about the most recent months of behaviour, not about recent-rate rules in general.

The recent-rate rule works for the same reason the model misses. The purchase rate falls through calibration (see *Why the model falls short*), and a rule that looks only at the end of the window picks up the lower rate. Rules that average the whole window overshoot: each customer's calibration rate gives +47.4% and the population rate +47.3%. Both correctly adjust for how long each customer was observed. Predicting that each customer repeats as often as last window gives +23.6%. That is closer, but only because it under-counts exposure: customers averaged 229 days of calibration against a 273-day holdout, and that bias partly cancels the fall in the rate.

On per-customer error the model has a lower MAE than all 5 naive rules: MAE 0.818, against 0.945 for the last-91-day rule and 0.911, 1.038 and 1.389 for the others. **Read it against predicting that nobody buys anything, which scores 0.835**, because 16,512 of these customers genuinely buy nothing. On a mostly-zero quantity that is the real floor, and the model clears it by 2.0%.

So the model's measurable advantage is narrow. Per customer it is modestly better than the naive rules. For the 14,119 customers with no repeat history, every naive rule predicts either zero or one figure shared by everybody. The model gives each of them an individual estimate, averaging 0.230 purchases against 0.251 actual. It also gives a structural account of why it misses. It is not more accurate on the total, and *Does the ranking work?* shows it is not better at ranking either.
...
| Naive: recent rate | 0.945 | 2.209 | 19,684 | 21,585 | 9.7 | 0.458 | not reported |
...
| Naive: recent rate | 38.140 | 119.485 | 776,961 | 852,676 | 9.7 | 0.455 | 149.6 |
...
| Model | MAE | RMSE | Spearman ρ | MAPE % |
| --- | --- | --- | --- | --- |
| BG/NBD + Gamma-Gamma | 18.142 | 31.357 | 0.390 | 63.4 |
| Naive: past average order value | 19.035 | 32.972 | 0.387 | 64.8 |
...
2,065 of those customers had no repeat purchase in calibration. The spend model and the naive rule both give them the same population mean, so on those rows the two cannot differ, and the comparison above is diluted towards a tie.
...
BG/NBD can only explain a falling purchase rate as customers dropping out, so it carries a decline forward. Holdout purchases by month, against the model's expectation for the same months:

| Month | Actual | Expected | Expected − actual |
| --- | --- | --- | --- |
| Oct 1997 | 2,397 | 2,281 | -116 |
| Nov 1997 | 2,620 | 2,092 | -528 |
| Dec 1997 | 2,395 | 2,058 | -337 |
| Jan 1998 | 1,965 | 1,964 | -1 |
| Feb 1998 | 1,957 | 1,703 | -254 |
| Mar 1998 | 2,686 | 1,814 | -872 |
| Apr 1998 | 1,806 | 1,691 | -115 |
| May 1998 | 1,903 | 1,687 | -216 |
| Jun 1998 | 1,955 | 1,578 | -377 |

From the first holdout month to the last, per day, the model's expectation changes by -28.5% and the realised rate by -15.7%. The model's decline is steeper than the cohort's, but that is not the whole story. The shortfall is uneven: Nov 1997, Mar 1998, Jun 1998 hold 63% of it. None of the three models has a seasonal term, so a seasonal pattern would show up exactly like this. A single year of holdout cannot confirm that it is seasonal.

Read the first three months of the chart as cohort formation rather than growth: every customer in this dataset makes their first purchase in Q1 1997, so the repeat series necessarily climbs while the cohort is still being acquired.
...
Purchases each fitted model missed, per calibration-repeat bucket. Negative means under-predicted.

| Model | 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7+ | Net | Sum of absolute misses |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| BG/NBD + Gamma-Gamma | -300 | -1,185 | -755 | -277 | -109 | -57 | -70 | -63 | -2,817 | 2,817 |
| MBG/NBD | -971 | -1,049 | -753 | -319 | -159 | -102 | -120 | -324 | -3,797 | 3,797 |
| Pareto/NBD | -1,423 | -532 | -217 | 88 | 136 | 98 | 66 | 305 | -1,479 | 2,864 |

**The net total and the bucket-level misses rank the models differently.** Pareto/NBD has the smallest net miss (-1,479), but its misses add up to 2,864 bucket by bucket, against 2,817 for BG/NBD + Gamma-Gamma. Its total is closer because its under- and over-predictions offset, not because it is closer in each bucket. So these fits do not show that a different dropout mechanism fixes the shortfall.

BG/NBD under-predicts in 8 of 8 buckets. A shortfall in every bucket is consistent with the purchase rate itself changing between the windows, which none of the three models represents.
...
**The honest reading is that almost all of that capture is available without the model.** The naive rule gets to within 1.1 percentage points of it, and on rank correlation it is actually **ahead**: Spearman ρ 0.479 against the model's 0.421 on revenue, and 0.485 against 0.442 on purchase counts. The naive ranking is each customer's calibration purchase rate times their past average order value, and the modelling does not improve on it.

The ordering is also mildly **inverted** at the bottom. Decile 10, the lowest-predicted tenth, realises a mean of 13.32 against 8.38-9.88 across deciles 5-9. Of its 2,357 customers, 1,069 (45%) are repeat buyers, with a mean probability alive of 0.20. They spent 17.76 on average in the holdout, against 9.63 for the one-time buyers in the same decile. The model ranks these repeat buyers at the very bottom, yet they spend more in the holdout than the one-time buyers ranked alongside them. That is the same write-off the frequency buckets show.
...
The paper reports the BG/NBD under-forecasting by 4% and the Pareto/NBD by 2% over the forecast period. **That figure is cumulative across all 78 weeks and must not be read against the holdout-only error above**: an in-sample fit accurate to within a percent over the first 39 weeks dilutes whatever happens in the second 39. On the paper's cumulative basis, this project's BG/NBD expects 41,400 repeat transactions against 44,021 actual: **-6.0%**, the figure to set beside the paper's. The two still differ in sample, so they are not expected to match exactly.
...
- **No uncertainty intervals.** This is a `map` fit, and the predictions table carries none. MAP returns a single point, so any interval computed from it would be zero-width and read as certainty. Intervals require `uv run ltv fit --full-bayes`.
